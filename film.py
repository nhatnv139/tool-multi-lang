...
def plan_transitions(scenes, opts=None):
    ...
    for i in range(n - 1):
        ...
        kind, dur = None, 0.0
        # KICH BAN V2 (1 canh = nhieu shot): hai shot CUNG canh -> CAT THANG, bat ke
        # always_fade. Truoc day moi shot la 1 "canh" -> dip-to-black moi ~13 giay,
        # man hinh chop den lien tuc — day chinh la cho "chuyen canh chua ngon".
        same_shot_scene = (cur.get("shot_scene") is not None
                           and cur.get("shot_scene") == nxt.get("shot_scene"))
        if manual in ("cut", "none") or (same_shot_scene and not manual):
...
def make_scene(scene, opts, i):
    ...
    if narrate and subs and narr_pre:
        ...
        import re as _re2
        _PAUSE = _re2.compile(r"^/{2,4}$")
        def _pause_sec(s):
            """Dong chi gom dau '/' = NGHI CO CHU DICH (khong doc, khong hien phu de).
            '//' 0.8s · '///' 1.5s · '////' 2.4s — dung cho khoang lang truoc cau chot."""
            m = _PAUSE.match(_sub_text(s))
            return {2: 0.8, 3: 1.5, 4: 2.4}[len(m.group(0))] if m else 0.0
        def _run_key(s):
            t = _sub_text(s)
            if not t or _pause_sec(s):
        ...
        for run in runs:
            ...
            # NGAT NGHI: noi bang "\n" chu KHONG phai " ".
            # Do thuc te tren Vbee (cung 1 cau): dau cach 2.83s -> xuong dong 3.41s,
            # tuc moi ranh gioi cau duoc +0.58s im lang. Dau "…" thi NGUOC LAI (2.64s,
            # doc nhanh hon) nen dung khong an thua. Day la thu duy nhat Vbee chiu nghi.
            joined = "\n".join(t for t in texts if t)
            k0 = run[0]
            _ps = _pause_sec(ss[0]) if len(ss) == 1 else 0.0
            if _ps or not joined:                # dau nghi '//' hoac cau trong -> im lang
                _sil = _ps or 1.4
                w = os.path.join(FILM, f"_sw_{i}_{k0}.wav")
                _seg_audio(None, _sil, w); tmp.append(w)
                seg_wavs.append(w); sub_dur.append(_sil)
                continue
            s0 = ss[0]
